...
def test_get_or_create_group_when_group_exists(mock_group, random_string):
    manager = Manager()
    manager._id_to_group[random_string] = mock_group

    return_value = manager.get_or_create_group(random_string)

    assert return_value == mock_group


@patch('pyloadover.manager.Group')
def test_get_or_create_group_when_group_not_exists(MockGroup: MagicMock, random_string):
    manager = Manager()
